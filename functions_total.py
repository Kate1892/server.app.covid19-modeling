...
def plot_smoothed_results(dataframe_with_results, smoothed_statistics, day_today, location, dday = None, test_data = None):
    titles = ['Number of daily diagnoses', 'Number of daily deaths','Number of ICU patients']
    colors = ['firebrick', 'black', '#b86113']

    i=0
    for suf in ['diagnoses', 'deaths', 'critical']:
        sub_data = dataframe_with_results[[c for c in dataframe_with_results.columns if suf in c]] # выбираем для отрисовки конкретную статистику из списка suf
        fig, ax = plt.subplots(figsize=(16,6))

        lag = cv.daydiff(smoothed_statistics.index[-1], dataframe_with_results.index[-1])


        if dday is not None:
            if test_data is not None:
                ax.scatter(smoothed_statistics.index[-dday+lag:-test_data],
                           smoothed_statistics.iloc[:,i][-dday+lag:-test_data],
                           color = colors[i], alpha=1, s = 25, label = 'Model data')

                ax.scatter(smoothed_statistics.index[-test_data:],
                           smoothed_statistics.iloc[:,i][-test_data:],
                           color = 'black', alpha=1, s = 25, label = 'Test data')


                ax.plot(sub_data.index[-dday:], sub_data.iloc[:,2][-dday:],
                        color = colors[i], linewidth = 2, label = 'Model results')

                ax.fill_between(sub_data.index[-dday:], sub_data.iloc[:,0][-dday:], sub_data.iloc[:,1][-dday:],
                                color = colors[i], alpha=0.2)

                plt.xticks(sub_data.index[-dday:], fontsize=18)
                ax.xaxis.set_major_locator(ticker.MultipleLocator(30))


            else:
                ax.scatter(smoothed_statistics.index[-dday+lag:], smoothed_statistics.iloc[:,i][-dday+lag:],
                           color = colors[i], alpha=1, s = 25, label = 'Data')
            ...
            if test_data is not None:
                ax.scatter(smoothed_statistics.index[:-test_data],
                           smoothed_statistics.iloc[:,i][:-test_data], color = colors[i],
                           alpha=1, s = 25, label = 'Model data')
                ...
                ax.plot(sub_data.index, sub_data.iloc[:,2], color = colors[i], linewidth = 2, label = 'Model results')
                ax.fill_between(sub_data.index, sub_data.iloc[:,0], sub_data.iloc[:,1], color = colors[i], alpha=0.2)

                plt.xticks(sub_data.index, fontsize=18)
                ax.xaxis.set_major_locator(ticker.MultipleLocator(90))

            else:
                ax.scatter(smoothed_statistics.index,
                           smoothed_statistics.iloc[:,i], color = colors[i], alpha=1, s = 25, label = 'Data')
                ax.plot(sub_data.index, sub_data.iloc[:,2], color = colors[i], linewidth = 2, label = 'Model results')
                ax.fill_between(sub_data.index, sub_data.iloc[:,0], sub_data.iloc[:,1], color = colors[i], alpha=0.2)
                plt.xticks(sub_data.index, fontsize=18)
                ax.xaxis.set_major_locator(ticker.MultipleLocator(90))
        ...
        plt.title(titles[i])
        plt.show()
        plt.savefig(f'{location}_{suf}_{day_today}.png', format='png')
        i+=1
...
def run_model(p, location, pop_location, start_day, end_day, b_days, b_changes, data, run,
              ...
              to_plot=['new_diagnoses','new_deaths', 'new_infections'], analyzers = None):
    ...
    if analyzers is not None:
    	print('analyzers')
    	sim = cv.Sim(pars, datafile=datafile, use_waning=True, analyzers=cv.daily_age_stats())
    else:
    	sim = cv.Sim(pars, datafile=datafile, use_waning=True)
...
def run_msim_conf(sim, to_plot, n_runs=1, save=True, namemsim=None, plot=True, jsonnamemsim=None):
    '''
    ...
    if save:
        ...
        #msim.to_json("Users_msim_res.json")
        msim.to_json(jsonnamemsim)
...
def prognose_check(df1, data_csv, location, pop_location, start_day, date1, date2, n_params, p, b_days, b_changes,
                    msimname, school_days=None, school_changes=None, run_sim=False, analyzers = None):
    '''
   ...
   Args:
    ...
    sim=run_model(p=p, location=location, pop_location=pop_location, start_day=start_day, end_day=date2,
                  ...
                  school_days=school_days, school_changes=school_changes, analyzers = analyzers)
...
def prognose(forecast_data, start_day, location, pop_location, p, to_plot, b_days, b_changes,
             school_days=None, school_changes=None, run_sim=False, n_runs=10, save=False, namemsim=None, plot=True, analyzers = None, jsonnamemsim=None): #поседний параметр
    '''
   ...
   Args:
    ...
    run_msim_conf(sim=sim, to_plot=to_plot, n_runs=n_runs, save=save, namemsim=namemsim, plot=plot, jsonnamemsim=jsonnamemsim)
...
def scenarios(forecast_data, p, location, pop_location, b_days,b_changes, school_days,
               school_changes, namescen,  start_day, n_runs=10, analyzers = None):
    # Run options
    end_date=cv.date(forecast_data.index[-1].to_pydatetime().date())
